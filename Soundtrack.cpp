...
{
	return ((composer == right.composer) || (title == right.title) || (label == right.label) 
		|| (catalogNumber == right.catalogNumber) || (dateRecorded == right.dateRecorded) || (dateReleased == right.dateReleased));
} 
